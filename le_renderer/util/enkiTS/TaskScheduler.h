...
class IPinnedTask : public ICompletable {
  public:
	IPinnedTask()
	    : threadNum( 0 ), pNext( NULL ) {
	} // default is to run a task on main thread
	IPinnedTask( uint32_t threadNum_ )
	    : threadNum( threadNum_ ), pNext( NULL ) {
	} // default is to run a task on main thread
...
class TaskSet : public ITaskSet {
  public:
	TaskSet() = default;
	TaskSet( TaskSetFunction func_ )
	    : m_Function( func_ ) {
	}
	TaskSet( uint32_t setSize_, TaskSetFunction func_ )
	    : ITaskSet( setSize_ ), m_Function( func_ ) {
	}
...
class TaskScheduler {
  ...
  private:
	...
	TaskPipe *      m_pPipesPerThread          = nullptr;
	PinnedTaskList *m_pPinnedTaskListPerThread = nullptr;
