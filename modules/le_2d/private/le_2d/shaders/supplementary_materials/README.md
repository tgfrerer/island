...
To pacify warnings when you set the output image format to anything other than rgba8 (which should be perfectly normal), we patch the spirv code that was generated by Tint. 

Wgsl does not allow unknown formats for output storage images, it requires you to specify a format; this means that the format of the output image must be known at shader compile time, which is not a luxury that we have...
