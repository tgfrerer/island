...
void TaskScheduler::AddTaskSetToPipe( ITaskSet *pTaskSet ) {
	...
	if ( pTaskSet->m_RangeToRun < pTaskSet->m_MinRange ) {
		pTaskSet->m_RangeToRun = pTaskSet->m_MinRange;
	}

	uint32_t rangeToSplit = pTaskSet->m_SetSize / m_NumInitialPartitions;
	if ( rangeToSplit < pTaskSet->m_MinRange ) {
		rangeToSplit = pTaskSet->m_MinRange;
	}
...
TaskScheduler::TaskScheduler()
    : m_pPipesPerThread( nullptr )
    , m_NumThreads( 0 )
    , m_pThreadNumStore( nullptr )
    , m_pThreads( nullptr )
    , m_bRunning( 0 )
    , m_NumThreadsRunning( 0 )
    , m_NumThreadsWaiting( 0 )
